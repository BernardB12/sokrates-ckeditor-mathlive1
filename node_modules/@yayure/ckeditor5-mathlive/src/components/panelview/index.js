...
export default class PanelView {
    ...
    mount(hookContainer) {
        ...
        const panelCommand = hookContainer[pluginScopeName].panelCommand;
        ...
        const handle = container.querySelector('.ck-mathlive-panel-handle');
        registerDragElement(container, handle);
        // Add interaction to the math-field element.
        const mathField = container.querySelector('.ck-mathlive-panel-input math-field');
        ...
        const confirmButton = container.querySelector('.ck-mathlive-panel-submit-confirm');
        confirmButton.addEventListener('click', () => {
            panelCommand.fire('insert', this.equation);
        });
        // Add interaction to cancel.
        const cancelButton = container.querySelector('.ck-mathlive-panel-submit-cancel');
        const closeButton = container.querySelector('.ck-mathlive-panel-header-close');
        const onClose = () => {
        ...
        // Add FormulaView.
        const formulaContainer = container.querySelector('.ck-mathlive-panel-formula');
...
export class FormulaView {
    constructor(props) {
        this.destroy = () => { };
        this.hookContainer = null;
        this.props = undefined;
        this.activeTabKey = 'SUPAndSUB-fraction-radical';
        this.formulaTabs = [{
        ...
        this.latexIconMarkupMap = undefined;
        this.latexIconConvertInterceptorMap = {
            '\\ce{#0}': {
                before: () => {
                    return '\\boxed{\\ce{▢}}';
                },
    ...
    mount(hookContainer) {
        ...
        texElements.forEach(element => {
            element.addEventListener('click', e => {
                const equation = e.target?.getAttribute('equation') || '';
			...
			<div class="ck-mathlive-formula-content">
				${latexIconMarkupMap ? formulaRows.map(equations => `<div class="ck-mathlive-formula-row">
					${equations.map(equation => `<div class="ck-mathlive-formula-tex ${activeTabKey}" equation="${equation}">
						<div class="ck-mathlive-latex-markup">${latexIconMarkupMap[equation]}</div>
					</div>`).join('')}
				</div>`).join('') : ''}
			</div>
		`;
...
function registerDragElement(container, handle) {
    let startClientX = 0;
    let startClientY = 0;
    let offsetX = 0;
    let offsetY = 0;
